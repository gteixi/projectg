  ...
  return (
    <>
      ...
      >
        ...
        <span className="text-sm font-semibold tabular-nums text-right text-gray-700 whitespace-nowrap">
          {total_produced} {unit}
        </span>
      </li>
          ...
          <ul className="divide-y divide-[#e5e3de]">
            {entries.map((e, i) => (
              <li key={i} className="grid grid-cols-[1fr_auto_auto] items-center gap-x-3 px-4 py-2.5 md:px-8 overflow-hidden">
                ...
                <span className="text-sm font-semibold tabular-nums text-gray-800 whitespace-nowrap text-right w-20">
                  {e.quantity} {e.unit}
                </span>
              </li>
            ))}
