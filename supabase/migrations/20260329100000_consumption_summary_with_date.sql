CREATE OR REPLACE FUNCTION get_consumption_summary(ref_date date DEFAULT CURRENT_DATE)
RETURNS TABLE (
  production_id uuid,
  name text,
  unit text,
  ...
  SELECT
    ...
    COALESCE((
      ...
      WHERE pl.production_id = p.id
        AND pl.logged_at >= (ref_date - interval '6 days')::date
        AND pl.logged_at < (ref_date + interval '1 day')::date
    ), 0) AS total_produit,
    COUNT(DISTINCT se.logged_at::date) AS dias_con_actividad
  FROM productions p
  JOIN stock_exits se
    ON se.production_id = p.id
   AND se.logged_at >= (ref_date - interval '6 days')::date
   AND se.logged_at < (ref_date + interval '1 day')::date
  GROUP BY p.id, p.name, p.unit, p.station;
$$;

CREATE OR REPLACE FUNCTION get_idle_productions(ref_date date DEFAULT CURRENT_DATE)
RETURNS TABLE (
  production_id uuid,
  name text,
  unit text,
